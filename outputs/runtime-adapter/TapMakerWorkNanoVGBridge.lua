...
local function request(method, route, payload, callback)
    ...
    client:OnError(function(_, status, message)
        ...
        -- Transient preview/bridge races must not kill HTTP forever; cooldown then retry.
        state.httpEnabled = false
        state.httpCooldownUntil = os.clock() + 2
...
local function installHooks()
    ...
    wrap("nvgEndFrame", function(base, ...)
        ...
        local result = base(...)
        -- Keep the latest frame for Update()'s interval push. Pushing HTTP
        -- snapshots from every EndFrame starves /api/runtime/commands and makes
        -- live-edit patches look like no-ops on NanoVG-only games.
...
function Bridge.Update(dt)
    ...
    -- Drain IDE patches before uploading snapshots so HTTP is not monopolized.
