...
>
> **macOS 限制：** 网上下载的未签名包需要正式 Developer ID + 公证才能「正常安装」。若 **系统设置 → 隐私与安全性** 里没有「仍要打开」，请按仓库根目录 `README.md`「下载 → macOS 安装说明」自行拉取源码打包（详细步骤见 `docs/DESKTOP_RELEASE.md` 文首）。

首次运行时输入 Maker 项目的绝对路径，或把项目文件夹拖进终端窗口再按回车。启动器会记住该路径。也可以将项目文件夹直接拖到启动器图标上。

如果 macOS 阻止打开启动器，可在 Finder 中右键启动器，选择“打开”；若设置里没有「仍要打开」，改用源码目录内直接运行本启动器（本仓拉取后的文件一般不受下载隔离影响）。
